...
eval_interval = 50     # 每多少轮打印一次损失
width = 50             # 文本生成时的换行宽度

device = "cuda" if torch.cuda.is_available() else "cpu"
torch.manual_seed(1337)

# 数据文件路径 (使用相对路径，确保在不同机器上都能运行)
file_name = "红楼梦.txt" 
...
def main():
    ...
    # 获取真实的后续文本 (Ground Truth)，其实这个部分并不需要也可以，因为 GPT 是一种 生成式模型 ，它的目标不是“背诵原文”，而是“模仿风格”，我们更希望看到模型能学到《红楼梦》的 文风 （比如半文半白、人物语气），而不是完全照抄。
